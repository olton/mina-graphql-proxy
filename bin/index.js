...
const {
  UrlLoader
} = require('@graphql-tools/url-loader');
...
`;
const hiddenFields = ["trackedWallets", "currentSnarkWorker", "initialPeers", "wallet"];
...
getSchema().then(function (remoteSchema) {
  const schema = wrapSchema({
    schema: remoteSchema,
    transforms: [// new FilterRootFields((operation, fieldName, field) => fieldName !== '_mutationType'),
    new FilterRootFields((operation, fieldName, field) => !field.isDeprecated), new FilterRootFields((operation, fieldName, field) => operation !== 'Mutation'), new FilterRootFields((operation, fieldName, field) => hiddenFields.includes(fieldName) === false)]
  });
  app.use(cors());
